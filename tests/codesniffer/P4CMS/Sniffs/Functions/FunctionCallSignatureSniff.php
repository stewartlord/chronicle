<?php
/**
 * Variation on PEAR version to accomodate anonymous functions.
  *
 * Derived from P4CMS_Sniffs_Functions_FunctionCallSignatureSniff:
 * @copyright 2006 Squiz Pty Ltd (ABN 77 084 670 600)
 * @license   http://matrix.squiz.net/developer/tools/php_cs/licence BSD Licence
 */
class P4CMS_Sniffs_Functions_FunctionCallSignatureSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function processMultiLineCall(PHP_CodeSniffer_File $phpcsFile, $stackPtr, $openBracket, $tokens)
    {
        ...
        for ($i = ($openBracket + 1); $i < $closeBracket; $i++) {
            ...
            if ($tokens[$i]['code'] === T_FUNCTION) {
                $openCurly = $phpcsFile->findNext(T_OPEN_CURLY_BRACKET, $i, $closeBracket);
                $i = $tokens[$openCurly]['bracket_closer'] + 1;
                if ($tokens[$i]['code'] === T_COMMA) $i++;
                continue;
            }
            ...
            if ($tokens[$i]['line'] !== $lastLine) {
                ...
                if ($tokens[$i]['line'] === $tokens[$closeBracket]['line']) {
                    ...
                    // Subsequent args must be aligned to the next tab stop and
                    // at least 2 spaces further indented.
                    $expectedIndent = $functionIndent - ($functionIndent % 4) + 4;
                    if ($expectedIndent - $functionIndent < 2) {
                        $expectedIndent += 4;
                    }
                }
        ...
        if ($tokens[($openBracket + 1)]['content'] !== $phpcsFile->eolChar) {
            ...
            // skip error if next line starts with anonymous function.
            $nextFunc = $phpcsFile->findNext(T_FUNCTION, $openBracket, $closeBracket);
            if (!$nextFunc || $tokens[$nextFunc]['line'] !== $tokens[$openBracket]['line'] + 1) {
                $phpcsFile->addError($error, $stackPtr);
            }
        }
