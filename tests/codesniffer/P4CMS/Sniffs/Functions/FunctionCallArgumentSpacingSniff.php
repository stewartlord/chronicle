 ...
 * Variation on PEAR version to skip the space after comma check.
 * Checks that calls to methods and functions are spaced correctly.
 *
 * Derived from PEAR_Sniffs_Functions_FunctionCallArgumentSpacingSniff:
 * @copyright 2006 Squiz Pty Ltd (ABN 77 084 670 600)
 * @license   http://matrix.squiz.net/developer/tools/php_cs/licence BSD Licence
 */
class P4CMS_Sniffs_Functions_FunctionCallArgumentSpacingSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
	    ...
	    // Make sure we are not inside an anonymous function argument.
            $prevFunction = $phpcsFile->findPrevious(T_FUNCTION, $nextSeperator, $openBracket);
            if ($prevFunction) {
                continue;
            }
            ...
            if ($tokens[$nextSeperator]['code'] === T_COMMA) {
                ...
                if ($tokens[($nextSeperator + 1)]['code'] !== T_WHITESPACE) {
                    ...
                    // If there is a newline in the space, then they must be
                    // formatting each argument on a newline, which is valid, so
                    // ignore it.
