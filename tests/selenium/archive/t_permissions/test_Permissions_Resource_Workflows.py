...
class test_Permissions_Resources_Workflows(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(connection["host"], connection["port"], connection["browser"], connection["url"])
        self.selenium.start()

    def test_permissions_resources_workflows(self):
        sel = self.selenium
        sel.open("/")
        ...
        sel.click("id=resource-resources-workflows")
        functions.waitForElementPresent(sel,"css=span.resourceLabel > Workflows")
        try: 
           self.failIf(sel.is_element_present("css=span.resourceLabel > Workflows"))
           functions.printResults("6541","pass")
        except AssertionError, e:
           self.verificationErrors.append(str(e))
           functions.printResults("6541","fail")
        try: 
