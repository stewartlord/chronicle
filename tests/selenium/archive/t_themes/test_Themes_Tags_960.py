...
class test_Themes_Tags_960(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(connection["host"], connection["port"], connection["browser"], connection["url"])
        self.selenium.start()

    def test_themes_tags_960(self):
        sel = self.selenium
        sel.open("/")
        ...
        sel.click("id=tagFilter-display-960")
        functions.waitForTextPresent(sel,"960 Blue")
        try: 
           self.failUnless(sel.is_text_present("960 Blue"))
           functions.printResults("6550","pass")
        except AssertionError, e:
           self.verificationErrors.append(str(e))
           functions.printResults("6550","fail")
        try: 
