...
class test_Modules_Tags_Media(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(connection["host"], connection["port"], connection["browser"], connection["url"])
        self.selenium.start()

    def test_modules_tags_media(self):
        sel = self.selenium
        sel.open("/")
        ...
        sel.click("id=tagFilter-display-media")
        functions.waitForTextPresent(sel,"Flickr")
        try: 
           self.failUnless(sel.is_text_present("Flickr"))
           functions.printResults("2302","pass")
        except AssertionError, e:
           self.verificationErrors.append(str(e))
           functions.printResults("2302","fail")
        try: 
