...
class test_Modules_Tags_Navigation(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(connection["host"], connection["port"], connection["browser"], connection["url"])
        self.selenium.start()

    def test_modules_tags_navigation(self):
        sel = self.selenium
        sel.open("/")
        ...
        sel.click("id=tagFilter-display-navigation")
        functions.waitForTextPresent(sel,"Category")
        try: 
           self.failUnless(sel.is_text_present("Category"))
           functions.printResults("2308","pass")
        except AssertionError, e:
           self.verificationErrors.append(str(e))
           functions.printResults("2308","fail")
        try: 
