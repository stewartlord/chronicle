...
class test_Modules_Tags_Users(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(connection["host"], connection["port"], connection["browser"], connection["url"])
        self.selenium.start()

    def test_modules_tags_users(self):
        sel = self.selenium
        sel.open("/")
        ...
        sel.click("id=tagFilter-display-users")
        functions.waitForTextPresent(sel,"User")
        try: 
           self.failUnless(sel.is_text_present("User"))
           functions.printResults("2318","pass")
        except AssertionError, e:
           self.verificationErrors.append(str(e))
           functions.printResults("2318","fail")
