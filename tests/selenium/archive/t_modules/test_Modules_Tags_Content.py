...
class test_Modules_Tags_Content(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(connection["host"], connection["port"], connection["browser"], connection["url"])
        self.selenium.start()

    def test_modules_tags_content(self):
        sel = self.selenium
        sel.open("/")
        ...
        sel.click("id=tagFilter-display-content")
        functions.waitForTextPresent(sel,"Category")
        try: 
           self.failUnless(sel.is_text_present("Category"))
           functions.printResults("2301","pass")
        except AssertionError, e:
           self.verificationErrors.append(str(e))
           functions.printResults("2301","fail")
        try: 
