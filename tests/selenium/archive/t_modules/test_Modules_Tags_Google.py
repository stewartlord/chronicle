...
class test_Modules_Tags_Google(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(connection["host"], connection["port"], connection["browser"], connection["url"])
        self.selenium.start()

    def test_modules_tags_google(self):
        sel = self.selenium
        sel.open("/")
        ...
        sel.click("id=tagFilter-display-google")
        functions.waitForTextPresent(sel,"Analytics")
        try: 
           self.failUnless(sel.is_text_present("Comment"))
           functions.printResults("6847","pass")
        except AssertionError, e:
           self.verificationErrors.append(str(e))
           functions.printResults("6847","fail")
